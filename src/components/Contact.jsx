            ...
            <div className="reveal space-y-5">
              {[
                ...
                <div key={i} className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-xl bg-purple-50 border border-purple-100 flex items-center justify-center text-purple-600 flex-shrink-0">{c.icon}</div>
                  <div>
                    <p className="text-xs text-gray-400 font-body mb-0.5 uppercase tracking-wider">{c.label}</p>
                    {c.href ? <a href={c.href} className="text-gray-800 font-body text-sm hover:text-purple-600 transition-colors">{c.value}</a>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {['student', 'university', 'recruiter'].map((r) => (
                        <button key={r} type="button" onClick={() => setForm({ ...form, role: r })}
                          className={`py-2 px-3 rounded-lg text-sm font-display font-600 border transition-all capitalize ${form.role === r ? 'bg-purple-600 text-white border-purple-600' : 'bg-gray-50 text-gray-500 border-gray-100 hover:border-purple-200 hover:text-purple-600'}`}>
                          {r}
                        </button>
                      ))}
                    </div>
